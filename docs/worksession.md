...
Next slice: refresh the sanitized TickTick Warung OS board cache (`npm run ticktick:collect`) if credentials are available, then regenerate/validate the public snapshot so the hosted mirror has current board data. Do not add auth unless Raz explicitly asks later.
