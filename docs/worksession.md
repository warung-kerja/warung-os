...
Next slice: surface TickTick board columns/task counts in Active Projects UI using the existing sanitized TickTick cache only; preserve privacy boundary (task id/title/column/priority/status/updated timestamp only) and do not read descriptions/comments. Later candidate: implement agent activity feed from Hermes session metadata.
