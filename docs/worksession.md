...
Next slice: identify and implement the next safe adapter or UI improvement. Candidates: (1) surface TickTick board columns/task counts in Active Projects UI; (2) add cron run-history chart to Operations Automation tab; (3) implement agent activity feed from Hermes session metadata.
