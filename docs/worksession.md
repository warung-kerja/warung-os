...
Next slice: Active Projects source-of-truth pass — surface TickTick board status more prominently in Active Projects list view; consider adding per-project status badge from snapshot `status` and `blocker` fields; optionally prompt for Obsidian frontmatter completeness (most project folders currently return `status: 'unknown'` since structured frontmatter is absent). No new data sources needed — use existing snapshot fields only.
