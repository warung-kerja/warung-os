...
Next slice: add hourly heatmap to session activity feed (requires per-hour SQL grouping in hermes-session-activity adapter); or add per-profile token usage breakdown to UsageTab.
