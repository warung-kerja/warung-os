...
Next slice: Blocked on Claude Code auth — the attempted Obsidian frontmatter seeding worker returned `401 Invalid authentication credentials` before making project changes. Refresh Claude Code auth before relaunching `claude-code`, or switch this slice to `mia-direct` for manual frontmatter seeding. Preserve the Phase 5 hosted/auth freeze: no auth, Supabase, Vercel, deployment, publisher, external account, or credential changes.
